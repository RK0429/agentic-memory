...
| `syncCheck()` | Values ストアの `promoted: true` とセクション内容の差分を検出。比較対象は `id`（存在有無）と `description`（投影後テキスト一致）。`description` は投影処理（200 文字切り詰め、改行置換、HTML コメントマーカーサニタイズ）を適用した値と比較する。`confidence` / `evidence` 件数はプロモーション時点のスナップショット値であり、同期チェックの対象外（後述の同期スコープ参照） |
...
| `description` | **対象**（投影後テキスト一致チェック） | 行動指針テキストの正確性。投影処理（200 文字切り詰め・改行置換・サニタイズ）を適用した値で比較 |
