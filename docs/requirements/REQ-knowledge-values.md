...
3. `knowledge/{domain}/{id}.md` にファイル作成（`id` は `k-` プレフィックス付きで自動生成済み）
...
**マーカー仕様:** `<!-- BEGIN:PROMOTED_VALUES ... -->` と `<!-- END:PROMOTED_VALUES -->` の HTML コメントマーカーで囲まれた範囲のみを自動管理対象とする。マーカーが欠落している場合は操作を拒否し、エラーメッセージで復元手順を案内する。
