...
| — | 2026-04-10 | must/should 指摘対応: REQ-FUNC-034 に降格提案の表面化経路（`memory_values_update` レスポンスの `demotion_candidate`）を追加、REQ-FUNC-009 出力節に `demotion_candidate` を追加、REQ-NF-001 ベンチマーク環境に GitHub Docs 出典 URL を追加 |
| — | 2026-04-10 | レビュー差し戻し対応: GitHub Actions 出典 URL を GitHub-hosted runners reference に修正 |
| — | 2026-04-10 | must/should 指摘対応（再レビュー）: REQ-NF-001 ベンチマーク環境を「CI 必須 / ラップトップ参考」に明確化（「いずれかで計測」と「どちらでも合格」の矛盾を解消） |
| — | 2026-04-10 | must/should 指摘対応（再々レビュー）: REQ-FUNC-012 処理節の照合キーを `title` + `domain` + `content` に修正し `create_new` / `skip_duplicate` を追加、REQ-FUNC-013 処理節の照合キーを `description` + `category` に修正し `create_new` / `skip_duplicate` を追加（DOMAIN/ARCH との全 outcome 整合） |
| — | 2026-04-12 | F-1 対応: MCP API の正式名を `origin` に統一し、legacy `source_type` を add 系 batch entry の互換 alias として明記。`origin`（エントリ全体の出自分類）と `sources[].type` / `Source.type`（個別参照の種別）の違いを明文化 |
...
| `origin` | enum | No | `memory_distillation` / `autonomous_research` / `user_taught`。エントリの主要な出自を示す。`sources[].type` とは独立に、エントリレベルの分類として使用する |
...
| 属性 | 型 | 説明 |
|---|---|---|
| `type` | enum | `memory_note` / `web` / `user_direct` / `document` / `code` / `other`。個々の引用元の種別を示す。エントリレベルの `origin` とは独立で、sources マージにより異なる type が混在し得る |
...
- **受け入れ基準**:
  ...
  - Given `content` にシークレットらしき文字列が含まれる, When `memory_knowledge_add` 実行, Then ラッパー層で `{ok: false, error_type: "validation_error", ...}` を返し、ファイルとインデックスは作成しない
- **優先度**: Must
- **出典**: ユーザー明示
...
| `accuracy` | enum | No | `verified` / `likely` / `uncertain`（デフォルト: `uncertain`） |
| `sources` | list[Source] | No | 引用元リスト |
| `origin` | enum | No | `memory_distillation` / `autonomous_research` / `user_taught`（legacy `source_type` も互換 alias として受理） |
...
**出力:** 成功時は作成されたエントリの `id` とパス。`content` でシークレット検出時はラッパー層で `validation_error` を返し、永続化は行わない。
...
| `origin` | enum | No | `memory_distillation` / `autonomous_research` / `user_taught`。エントリ全体の主要な出自を示す（デフォルト: `user_taught`） |
...
- **受け入れ基準**:
  ...
  - Given `description` または `evidence[].summary` にシークレットらしき文字列が含まれる, When `memory_values_add` 実行, Then ラッパー層で `{ok: false, error_type: "validation_error", ...}` を返し、ファイルとインデックスは作成しない
- **優先度**: Must
- **出典**: ユーザー明示
...
| `origin` | enum | No | `memory_distillation` / `autonomous_research` / `user_taught`（デフォルト: `user_taught`。legacy `source_type` も互換 alias として受理） |

**処理:**
1. `description` と `evidence[].summary` に対してラッパー層でシークレット検出を行い、検出時は `validation_error` を返して処理を中断する
2. UUID を生成し `v-` プレフィックスを付与して `id` とする（作成時のみ。以降不変）
3. 厳密重複チェック: `description` + `category` が既存エントリと実質同一の場合はエラーを返す（ID ではなく内容ベースで判定。Knowledge の REQ-FUNC-004 と同じ方針）
4. 類似判定: 既存 Values と意味的に重複する場合は警告を返す（エラーではなく、マージ提案。厳密重複とは別のチェック。アルゴリズムは下記「類似判定の仕様」参照）
5. `evidence` 初期化: `evidence` パラメータが指定された場合、提供リストの先頭10件を保持し末尾を切り捨てる（10件以下ならそのまま保持。リスト先頭が最新として扱われる）。`evidence_count` は指定された `evidence` の件数（切り捨て前の総数）で初期化する（未指定時は 0）
6. 昇格候補判定: 初期 `confidence` と `evidence_count` が昇格条件（REQ-FUNC-015: `confidence >= 0.8` AND `evidence_count >= 5`）を満たす場合、レスポンスに昇格候補である旨を通知する（REQ-FUNC-009 の更新時通知と同じ形式）
7. `values/{id}.md` にファイル作成（`id` は `v-` プレフィックス付きで自動生成済み）
8. `_values.jsonl` にインデックスエントリを追加
...
**出力:** 成功時は作成されたエントリの `id`。昇格候補条件（REQ-FUNC-015）を満たす場合は `promotion_candidate: true` を返す（満たさない場合はフィールド自体を省略）。類似既存エントリがある場合はその情報を `warnings` に含める（例: `"Similar value exists: v-xxxxxxxx — {description}"`）。`description` または `evidence[].summary` でシークレット検出時はラッパー層で `validation_error` を返し、永続化は行わない。
...
**出力:** 更新されたエントリの `id`。更新後に昇格候補条件（REQ-FUNC-015）を満たす場合は `promotion_candidate: true` を返す（満たさない場合はフィールド自体を省略）。`promoted: true` のエントリで降格提案条件（REQ-FUNC-034: `confidence` が `promoted_confidence` から 0.2 以上低下）を満たす場合は `demotion_candidate: true` を返す（満たさない場合はフィールド自体を省略）。
...
**処理:**
1. 新規候補の `title` + `domain` + `content` と既存エントリを照合
2. 同一トピックと判定された場合（`merge_existing`）: 既存エントリの `content` を補完・拡張（矛盾する場合は `accuracy: uncertain` にフラグし、矛盾の内容を `conflictDetail` として記録）、`sources` に新しい引用元を追加、`updated_at` を更新
3. 類似だが別トピックと判定された場合（`link_related`）: 候補を新規エントリとして登録した後、新規エントリと既存エントリの `related` を相互に追加する（双方向リンク）
4. 該当する既存エントリがない場合（`create_new`）: 候補を新規エントリとして登録
5. 既存エントリと厳密重複の場合（`skip_duplicate`）: 登録をスキップ
...
**処理:**
1. 新規候補の `description` + `category` と既存エントリを照合
2. 同一傾向と判定された場合（`reinforce_existing`）: `confidence` を更新（同じ傾向の evidence が増えるほど上昇）、`evidence` に新事例を追加（最新10件保持ルール適用）
3. 矛盾する傾向が検出された場合（`contradict_existing`）: 既存エントリの `confidence` を低下させ、矛盾の内容を `contradictionDetail` として記録しレスポンスに報告
4. 該当する既存エントリがない場合（`create_new`）: 候補を新規エントリとして登録
5. 既存エントリと厳密重複の場合（`skip_duplicate`）: 登録をスキップ
...
- **受け入れ基準**:
  - Given エージェントが調査で事実を確認した, When `memory_knowledge_add` を `origin: autonomous_research` で呼び出す, Then Knowledge エントリが作成される
...
3. `memory_knowledge_add` で登録（`origin` と `accuracy` をファクトチェック結果に応じて設定）
...
- **受け入れ基準**:
  - Given `confirm: false` または未指定, When `memory_knowledge_delete` 実行, Then 削除プレビュー（`preview: true`, `would_delete: true`）のみを返し、ファイル・インデックス・`related` バックリンクは変更しない
  - Given `confirm: true`, When `memory_knowledge_delete` 実行, Then 実際に削除し、`related` バックリンクもクリーンアップする
...
| `confirm` | bool | No | 削除確認フラグ（デフォルト: `false`）。`false` または未指定ではプレビューのみを返し、実削除は行わない |
| `reason` | string | No | 削除理由 |

**処理:**
1. `confirm == true` を検証する。`false` または未指定の場合は削除プレビュー（`deleted_id`, `title`, `preview: true`, `would_delete: true`）を返し、ファイル・インデックス・`related` バックリンクは変更しない
2. `confirm == true` の場合、他エントリの `related` から対象 ID を除去する
3. `knowledge/{id}.md` と `_knowledge.jsonl` を削除する

**出力:** `confirm: false` または未指定では削除プレビュー（`deleted_id`, `title`, `preview: true`, `would_delete: true`）。`confirm: true` では削除結果（`deleted_id`, `title`, `deleted: true`）。`reason` が指定された場合は preview / 実削除のいずれでもレスポンスにエコーバックする。`reason` はレスポンス以外には永続化しない。削除理由の保存が必要な場合はエージェント側でセッションノートや `memory_state_add` に記録する。
...
- **受け入れ基準**:
  - Given ユーザーが「ハミルトン方程式は正準方程式とも呼ばれ…」と教えた, When エージェントが Knowledge 化, Then `origin: user_taught` で登録される
...
4. `memory_knowledge_add` で登録（`origin: user_taught`）

**ファクトチェック結果の永続化:** エントリレベルの `origin` は `user_taught`（知識の出自を示す）を維持する。`sources` リストには以下を含める:
- ユーザー教示の Source: `type: user_direct`, `ref`: セッション参照, `summary`: ユーザーの発言要約
- ファクトチェック結果の Source（検証を実施した場合）: `type: web` / `document` / `code` / `other` のいずれか、`ref`: 確認に使用した URL やドキュメント参照, `summary`: 検証結果の要約

これにより、エントリレベルの `origin`（出自分類）と個別 `sources[].type`（各引用元の種別）が独立に管理され、ファクトチェックの根拠が追跡可能になる。
...
- **説明**: Knowledge エントリの統計情報（総数、ドメイン別分布、accuracy 別分布、user_understanding 別分布、origin 別分布）を返す。
- **優先度**: Could
- **出典**: エージェント推測
...
- **受け入れ基準**:
  ...
  - Given `promoted: true` かつ `confidence` が `promoted_confidence` から 0.2 以上低下した Values エントリ, When `memory_values_update` のレスポンス, Then `demotion_candidate: true` を返す（REQ-FUNC-015 の `promotion_candidate` と同形式。条件を満たさない場合はフィールド自体を省略）
- **優先度**: Could
- **出典**: エージェント推測
- **関連要件**: REQ-FUNC-016, REQ-FUNC-022
...
- **ベンチマーク環境**: CI ランナーを必須の計測環境とし、開発者ラップトップは参考値とする:
  - CI ランナー（**必須**）: GitHub Actions 標準ランナー（`ubuntu-latest`）。ハードウェア仕様はリポジトリ可視性により異なる（private: 2 vCPU / 8 GB RAM、public: 4 vCPU / 16 GB RAM。2026-04-09 時点、[GitHub Docs: GitHub-hosted runners reference](https://docs.github.com/en/actions/reference/runners/github-hosted-runners) 準拠）。どちらの仕様でも基準を満たすこと
  - 開発者ラップトップ（**参考**）: Apple Silicon（M1 以降）または x86-64、8GB+ RAM、SSD ストレージ。合否判定には使用しない
...
- **基準**: Knowledge/Values エントリに対してシークレット・認証情報・API キーの検出バリデーションを行う。検出時のポリシーは操作の不可逆度に応じて段階的に適用する:
  - `memory_knowledge_add`: `content` に対してラッパー層で機密検出を行い、検出時は `{ok: false, error_type: "validation_error", ...}` を返して保存を拒否する
  - `memory_values_add`: `description` と `evidence[].summary` に対してラッパー層で機密検出を行い、検出時は `{ok: false, error_type: "validation_error", ...}` を返して保存を拒否する
  - `memory_knowledge_update` / `memory_values_update`: 警告をレスポンスに含めるが、保存は継続する（エージェントが警告に基づいて修正判断を行う想定）
