...
| **蒸留（Distillation）** | Memory ノート群およびローリングステート（`_state.md`）の「主要な判断」セクションから、一般化可能な Knowledge や Values を抽出するプロセス |
...
- 初回操作時に作成するもの（lazy）: `_knowledge.jsonl`（初回 `memory_knowledge_add` 時）、`_values.jsonl`（初回 `memory_values_add` 時）、`_state.md` の蒸留日時フロントマター（各フィールドは更新条件を初めて満たした時点で遅延追加。`last_*_evaluated_at` は初回 `dry_run=false` 蒸留完了時、`last_*_distilled_at` は初回の永続化発生時に追加される）
...
4. `evidence` 初期化: `evidence` パラメータが指定された場合、最新10件保持ルールを適用する（10件以下ならそのまま保持）。`evidence_count` は指定された `evidence` の件数で初期化する（未指定時は 0）
...
**出力:** 作成されたエントリの `id`。類似既存エントリがある場合はその ID と description も返す。昇格候補条件を満たす場合はその旨も返す。
...
**出力:** 抽出された Knowledge 候補のリスト（新規 / 既存マージ / 関連リンク / 重複スキップ を区別）と集計（`new` / `merged` / `linked` / `skipped`）
...
**日付パラメータの仕様:** `memory_distill_knowledge` と共通。REQ-FUNC-010 の「日付パラメータの仕様」を参照。
...
**出力:** 抽出された Values 候補のリスト（新規 / 強化（確信度更新） / 矛盾 / スキップ を区別）と集計（`new` / `reinforced` / `contradicted` / `skipped`）
...
3. Values エントリの `promoted: true`, `promoted_at`, `promoted_confidence`（昇格時点の `confidence` 値）を更新
...
2. 最終評価日時から 7 日間（168 時間）以上経過（タイムスタンプ精度で比較。日付境界ではなく経過時間で判定）
...
- `last_knowledge_distilled_at` / `last_values_distilled_at`: 最終永続化日時。`dry_run=false` かつ 1 件以上の永続化（create / merge / link / reinforce）が発生した蒸留完了時にのみ更新される。矛盾検出による `confidence` 低下（`CONTRADICT_EXISTING`）は永続化にカウントしない
