...
| 属性 | 型 | 必須 | 説明 |
|---|---|---|---|
| `id` | string | 自動 | 一意識別子（作成時に `title` + `domain` + `content[:100]` から一度だけ生成。プレフィックス `k-`。以降は不変） |
...
**処理:**
1. `title` + `domain` + `content[:100]` から `id` を生成（作成時のみ。以降不変）
2. 重複チェック（同一 `id` が存在すればエラー）
3. `knowledge/{id}.md` にファイル作成（`id` は `k-` プレフィックス付きで自動生成済み）
...
| 属性 | 型 | 必須 | 説明 |
|---|---|---|---|
| `id` | string | 自動 | 一意識別子（作成時に `description` + `category` から一度だけ生成。プレフィックス `v-`。以降は不変） |
...
**2 つの起動経路:**
- *公開 API ベースの推奨判定*（セッション終了時の振り返りおよび retrospective スキルで評価される）: 上記条件（10 ノート以上 OR 7 日以上経過）を評価し、満たす場合に蒸留を推奨する
...
- `last_knowledge_distilled_at` / `last_values_distilled_at`: 最終永続化日時。`dry_run=false` かつ 1 件以上の永続化（create / merge / reinforce）が発生した蒸留完了時にのみ更新される。矛盾検出による `confidence` 低下（`CONTRADICT_EXISTING`）は永続化にカウントしない
