...
- **ベンチマーク環境**: 以下のいずれかの環境で計測する。どちらの環境でも基準を満たすこと:
  - CI ランナー: GitHub Actions 標準ランナー（`ubuntu-latest`）。ハードウェア仕様はリポジトリ可視性により異なる（private: 2 vCPU / 8 GB RAM、public: 4 vCPU / 16 GB RAM。2026-04-09 時点、[GitHub Docs: GitHub-hosted runners reference](https://docs.github.com/en/actions/reference/runners/github-hosted-runners) 準拠）。どちらの仕様でも基準を満たすこと
