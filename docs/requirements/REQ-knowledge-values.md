...
**MCP ツール共通規約:** 以降で定義する全 MCP ツールは、既存ツールと同様に `memory_dir`（任意、記憶ディレクトリの指定）パラメータを受け取る。成功レスポンスは `ok` / `warnings` フィールドを含むエンベロープ形式で返す。各ツール定義では共通パラメータを省略し、ツール固有のパラメータのみを記載する。
